...
export default function FormulaCalculator({ calculatorId = 'formula-calc', onCalculation }) {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const response = await formulaService.getFormulas(filters);
                setFormulas(Array.isArray(response) ? response : (response?.formulas || []));
            } catch (err) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                setSelectedFormula(f?.formula || f || null);

                const o = await formulaService.getFormulaCalculationOptions(selectedId);
                const opts = Array.isArray(o) ? o : (o?.calculationOptions || []);
                setOptions(opts);

                if (opts.length) {
                    setOutputVar(opts[0].outputVariable);
                    const init = {};
                    (opts[0].requiredInputs || []).forEach(input => {
    ...
    useEffect(() => {
        ...
        if (!opt) return;
        const next = {};
        (opt.requiredInputs || []).forEach(input => {
            const key = typeof input === 'string' ? input : input.key;
            next[key] = inputs[key] ?? '';
        });
        setInputs(next);
    ...
    const validate = () => {
        ...
        for (const input of opt.requiredInputs || []) {
            const k = typeof input === 'string' ? input : input.key;
            const raw = inputs[k];
    ...
    const onCalculate = async () => {
        ...
        opt.requiredInputs.forEach(input => {
            const key = typeof input === 'string' ? input : input.key;
            payload.inputs[key] = Number(inputs[key]);
        });
                    ...
                    <section className="fc-work">
                        ...
                        {outputVar && (
                            <div className="inputs-grid">
                                {options.find(o => o.outputVariable === outputVar)?.requiredInputs?.map((input) => {
                                    const k = typeof input === 'string' ? input : input.key;
